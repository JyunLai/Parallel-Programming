...
float Microstructure::CalcReorientationRatesForFullFieldRexOrGG(int latticeId)
{
    ...
    int numInterfaceCells = interfaceIndicesVec.size();

    LOG_F(INFO, "calculating reorientation rates, number of cells so far where growth/rex initiated are %d", cells_rx);

    double MisTol = mvUserSettings.mvLowerMisorientationCutOff;
    double MisTolMoving = mvUserSettings.mvLowerMisorientationForLAGB;
    ...
    #pragma omp parallel for schedule(dynamic) reduction(max: fDotMaxICellsGroup) reduction(+: iCells, cellsMovingAlready, cellsMovingNew)
    for (int k = 0; k < numInterfaceCells; ++k) {
        int index = interfaceIndicesVec[k];
        ...
        double Component_X_OfUnitVectorOfForceAthBoundary, Component_Y_OfUnitVectorOfForceAthBoundary, Component_Z_OfUnitVectorOfForceAthBoundary;
        double CurrentStateEnergyPerUnitVolume, EnergyPerVolForThisBoundaryOfCurrentState;
        double NextStateEnergyPerUnitVolume, EnergyPerVolForThisBoundaryOfNextState;
        double TotalForceX_PerVol, TotalForceY_PerVol, TotalForceZ_PerVol;
        double DG_ForReorientation, MobilityOfBoundaries, TotalForceForOrientationPerVol, fDot;
        ...
        if (mvUserSettings.mvFasterRexAndGG) {
            ...
            if (mvpStateVars->mvpConsumptionRate[index] >= -0.01)
                continue;
        }
        ...
        for (int nbr = 0; nbr < NNeighbours; nbr++) {
            ...
            AreaPerVolumeForThisBoundary = GetAreaPerVolumeOfBoundary(index, indexNbrToGrow);
            CalculateDirectionOfForceAtThisBoundary(index, indexNbrToGrow, &Component_X_OfUnitVectorOfForceAthBoundary, &Component_Y_OfUnitVectorOfForceAthBoundary, &Component_Z_OfUnitVectorOfForceAthBoundary);
            TotalForceX_PerVol += (Component_X_OfUnitVectorOfForceAthBoundary * AreaPerVolumeForThisBoundary);
            TotalForceY_PerVol += (Component_Y_OfUnitVectorOfForceAthBoundary * AreaPerVolumeForThisBoundary);
            TotalForceZ_PerVol += (Component_Z_OfUnitVectorOfForceAthBoundary * AreaPerVolumeForThisBoundary);
            ...
            for (int nbr1 = 0; nbr1 < NNeighbours; nbr1++) {
                ...
                if (mis[nbr][nbr1] < MisTol) {
                    if (mvpStateVars->mvpRX[indexNbrOther] == 1 || (mvpStateVars->mvpHasNeighNonInd[indexNbrOther] == 0 && mvpStateVars->mvpCI[indexNbrOther] >= mvUserSettings.mvMinimumCI)) {
                        ...
                        CalculateDirectionOfForceAtThisBoundary(index, indexNbrOther, &Component_X_OfUnitVectorOfForceAthBoundary, &Component_Y_OfUnitVectorOfForceAthBoundary, &Component_Z_OfUnitVectorOfForceAthBoundary);
                        TotalForceX_PerVol += (Component_X_OfUnitVectorOfForceAthBoundary * AreaPerVolumeForThisBoundary);
                        TotalForceY_PerVol += (Component_Y_OfUnitVectorOfForceAthBoundary * AreaPerVolumeForThisBoundary);
                        TotalForceZ_PerVol += (Component_Z_OfUnitVectorOfForceAthBoundary * AreaPerVolumeForThisBoundary);
                    }
            ...
            TotalForceForOrientationPerVol = sqrt(pow(TotalForceX_PerVol, 2.) + pow(TotalForceY_PerVol, 2.) + pow(TotalForceZ_PerVol, 2.));

            MobilityOfBoundaries = GetMobilityForThisMisorientation(mvpStateVars->GetLatticeIdOfCell(index), mis[nbr][nbr]);

            fDot = TotalForceForOrientationPerVol * MobilityOfBoundaries;
        ...
        if (mvpStateVars->mvpConsumptionRate[index] < 0.0)
            mvpStateVars->mvpConsumptionRate[index] = 0.;
    ...
    LOG_IF_F(INFO,
            iCells > 0,
            "number of iCells %d for lattice id %d for max mob in [meter4  / J s]  %g sigma Max %g and kinetic factor beta %g fDotMax %g A/V %g cells moving pre %d and now also %d",
            iCells,
            latticeId,
