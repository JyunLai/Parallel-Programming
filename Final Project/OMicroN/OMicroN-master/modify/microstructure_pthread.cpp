...
// [Pthreads Additions] 定義傳遞給執行緒的參數結構
struct ThreadArgs {
    Microstructure* instance;       // 指向 Microstructure 物件的指標 (為了存取成員變數)
    int latticeId;                  // 函式參數
    std::vector<int>* indices;      // 要處理的 Cell Index 列表
    std::atomic<int>* globalIndex;  // 全域計數器 (用於動態排程)
    
    // --- Thread Local Results (每個執行緒的局部結果) ---
    float localMaxRate;
    int localMovingPre;
    int localMovingNew;
    int localICells;
    
    ThreadArgs() : localMaxRate(0.0), localMovingPre(0), localMovingNew(0), localICells(0) {}
};
...
// [Pthreads Additions] Worker 函式 (執行緒入口點)
void* Microstructure::ReorientationWorker(void* arg) {
    ThreadArgs* args = (ThreadArgs*)arg;
    Microstructure* ms = args->instance; // 取得物件實例
    const std::vector<int>& indices = *args->indices;
    int totalSize = indices.size();
    
    // [Chunk Size] 每次搶 64 個任務，減少 atomic 競爭開銷
    const int CHUNK_SIZE = 64; 
    int start_idx;

    // 動態排程迴圈：不斷搶下一批工作
    while ((start_idx = args->globalIndex->fetch_add(CHUNK_SIZE)) < totalSize) {
        ...
        for (int k = start_idx; k < end_idx; ++k) {
            ...
            // ==========================================================
            // 以下為原本 CalcReorientationRates... 內部的計算邏輯
            // 注意：所有的成員變數存取 (如 mvpStateVars) 都要改成 ms->
            // ==========================================================
            
            // 變數宣告 (Thread Local)
            int indexNbrToGrow, indexNbrOther, indexDragged;
            int lall[26], lall2[26];
            int OriIdToConsume, OriIdToGrow, OriIdNbr;
            int CSLRelationship;
            double AreaPerVolumeForThisBoundary, IncSigma;
            double Component_X, Component_Y, Component_Z;
            double CurrentStateEnergyPerUnitVolume, EnergyPerVolCurrent;
            double NextStateEnergyPerUnitVolume, EnergyPerVolNext;
            double TotalForceX, TotalForceY, TotalForceZ;
            double DG_ForReorientation, MobilityOfBoundaries, TotalForceForReOrientation, fDot;
            double mis[26][26]; 
            
            // 檢查 Lattice ID
            if (ms->mvpStateVars->GetLatticeIdOfCell(index) != args->latticeId)
                continue;
            
            args->localICells++;

            double MisTol = ms->mvUserSettings.mvLowerMisorientationCutOff;
            double MisTolMoving = ms->mvUserSettings.mvLowerMisorientationForLAGB;

            // Optimization for FasterRexAndGG
            if (ms->mvUserSettings.mvFasterRexAndGG){ 
                if (ms->mvpStateVars->mvpConsumptionRate[index] > 0.) args->localMovingPre++;
                
                // 更新局部最大值
                if (ms->mvpStateVars->mvpConsumptionRate[index] > args->localMaxRate)
                    args->localMaxRate = ms->mvpStateVars->mvpConsumptionRate[index];  
                
                if (ms->mvpStateVars->mvpConsumptionRate[index] >= -0.01) continue; 
            }

            if (!ms->mvUserSettings.mvAllowPeriodicBoundConditions && ms->mvpStateVars->mvpBoundaryCell[index])
                continue;

            OriIdToConsume = ms->mvpStateVars->GetOriIdOfCell(index);
            ms->mvpStateVars->mvpOneOfTheNeighboursGrowingInto[index] = -1;
            ms->mvpStateVars->mvpConsumptionRate[index] = 0;
            
            if (ms->mvUserSettings.mvIncludeCSL19FastGrowth)
                ms->mvpStateVars->mvpCSLRelationshipNow[index] = 0;

            ms->mvpStateVars->GetNeighbourCellOffsets(index, lall);
            
            int NNeighbours = ms->mvpStateVars->mvNAll;
            if (ms->mvUserSettings.mvGridType == 0 && ms->mvpStateVars->mvIs2DSimulation && ms->mvUserSettings.mvTestVonNeumannInSquare) {
                NNeighbours = ms->mvpStateVars->mvNNearest;
            }

            CurrentStateEnergyPerUnitVolume = 0.; 

            // --- 內層迴圈 1: 計算當前能量 ---
            for (int nbr = 0; nbr < NNeighbours; nbr++) {
                indexNbrToGrow = lall[nbr];
                OriIdToGrow = ms->mvpStateVars->GetOriIdOfCell(indexNbrToGrow);
                mis[nbr][nbr] = 0.;

                if (OriIdToConsume != OriIdToGrow) {
                    mis[nbr][nbr] = ms->MisorientationBetweenCells(index, indexNbrToGrow, true, false, ms->mvUserSettings.mvIncludeCSL19FastGrowth, &CSLRelationship);
                    if (CSLRelationship > 0) {
                        ms->mvpStateVars->mvpCSLRelationshipNow[index] = 19;
                        // 注意：這裡是 Race Condition，但在這個演算法中通常被接受，或者需要 atomic
                        // 為了效能，這裡保持原樣 (Risk: Multiple threads writing same value)
                        ms->mvpStateVars->mvpCSLRelationshipNow[indexNbrToGrow] = 19; 
                    }
                }

                if (!ms->mvUserSettings.mvAllowPeriodicBoundConditions && (fabs(ms->mvpStateVars->GetXFromIndex(index) - ms->mvpStateVars->GetXFromIndex(indexNbrToGrow)) > 5. * ms->mvpStateVars->mvDx || fabs(ms->mvpStateVars->GetYFromIndex(index) - ms->mvpStateVars->GetYFromIndex(indexNbrToGrow)) > 5. * ms->mvpStateVars->mvDx))
                    mis[nbr][nbr] = 0.;

                for (int nbr1 = 0; nbr1 < NNeighbours; nbr1++) {
                    if (nbr1 == nbr) continue;
                    indexNbrOther = lall[nbr1];

                    if (!ms->mvUserSettings.mvAllowPeriodicBoundConditions && (fabs(ms->mvpStateVars->GetXFromIndex(indexNbrOther) - ms->mvpStateVars->GetXFromIndex(indexNbrToGrow)) > 5. * ms->mvpStateVars->mvDx || fabs(ms->mvpStateVars->GetYFromIndex(indexNbrOther) - ms->mvpStateVars->GetYFromIndex(indexNbrToGrow)) > 5. * ms->mvpStateVars->mvDx))
                        mis[nbr][nbr1] = 0.;
                    
                    OriIdNbr = ms->mvpStateVars->GetOriIdOfCell(indexNbrOther);
                    if (nbr1 < nbr) {
                    ...
                    mis[nbr][nbr1] = ms->MisorientationBetweenCells(indexNbrToGrow, indexNbrOther, true, false, false, &CSLRelationship);
                }

                if (mis[nbr][nbr] < MisTol) continue; 
                
                IncSigma = ms->GetBoundaryEnergyForThisMisorientation(mis[nbr][nbr]);
                AreaPerVolumeForThisBoundary = ms->GetAreaPerVolumeOfBoundary(index, indexNbrToGrow);
                EnergyPerVolCurrent = AreaPerVolumeForThisBoundary * IncSigma;
                CurrentStateEnergyPerUnitVolume += EnergyPerVolCurrent;
            }
            ...
            for (int nbr = 0; nbr < NNeighbours; nbr++) {
                ...
                TotalForceX = 0.; TotalForceY = 0.; TotalForceZ = 0.;

                indexNbrToGrow = lall[nbr];
                if (!ms->mvUserSettings.mvAllowPeriodicBoundConditions && (fabs(ms->mvpStateVars->GetXFromIndex(index) - ms->mvpStateVars->GetXFromIndex(indexNbrToGrow)) > 5. * ms->mvpStateVars->mvDx || fabs(ms->mvpStateVars->GetYFromIndex(index) - ms->mvpStateVars->GetYFromIndex(indexNbrToGrow)) > 5. * ms->mvpStateVars->mvDx))
                    continue;

                if (ms->mvpStateVars->mvpRX[indexNbrToGrow] == 0 && (ms->mvpStateVars->mvpCI[index] < ms->mvUserSettings.mvMinimumCI || ms->mvpStateVars->mvpCI[indexNbrToGrow] < ms->mvUserSettings.mvMinimumCI || ms->mvpStateVars->mvpHasNeighNonInd[index] == 1 || ms->mvpStateVars->mvpHasNeighNonInd[indexNbrToGrow] == 1))
                    continue;
                if (mis[nbr][nbr] < MisTolMoving)
                    continue; 

                AreaPerVolumeForThisBoundary = ms->GetAreaPerVolumeOfBoundary(index, indexNbrToGrow);
                ms->CalculateDirectionOfForceAtThisBoundary(index, indexNbrToGrow, &Component_X, &Component_Y, &Component_Z);
                TotalForceX += (Component_X * AreaPerVolumeForThisBoundary);
                TotalForceY += (Component_Y * AreaPerVolumeForThisBoundary);
                TotalForceZ += (Component_Z * AreaPerVolumeForThisBoundary);
                ...
                for (int nbr1 = 0; nbr1 < NNeighbours; nbr1++) {
                    ...
                    if (mis[nbr][nbr1] < MisTol) {
                        if (ms->mvpStateVars->mvpRX[indexNbrOther] == 1 || (ms->mvpStateVars->mvpHasNeighNonInd[indexNbrOther] == 0 && ms->mvpStateVars->mvpCI[indexNbrOther] >= ms->mvUserSettings.mvMinimumCI)) {
                            AreaPerVolumeForThisBoundary = ms->GetAreaPerVolumeOfBoundary(index, indexNbrOther);
                            ms->CalculateDirectionOfForceAtThisBoundary(index, indexNbrOther, &Component_X, &Component_Y, &Component_Z);
                            TotalForceX += (Component_X * AreaPerVolumeForThisBoundary);
                            TotalForceY += (Component_Y * AreaPerVolumeForThisBoundary);
                            TotalForceZ += (Component_Z * AreaPerVolumeForThisBoundary);
                        }
                    } else {
                        IncSigma = ms->GetBoundaryEnergyForThisMisorientation(mis[nbr][nbr1]);
                        AreaPerVolumeForThisBoundary = ms->GetAreaPerVolumeOfBoundary(index, indexNbrOther);
                        EnergyPerVolNext = AreaPerVolumeForThisBoundary * IncSigma;

                        ms->mvpStateVars->GetNeighbourCellOffsets(indexNbrOther, lall2);
                        indexDragged = lall2[nbr];

                        if (ms->mvpStateVars->mvpRX[indexDragged] == 1 || (ms->mvpStateVars->mvpCI[indexNbrOther] >= ms->mvUserSettings.mvMinimumCI && ms->mvpStateVars->mvpCI[indexDragged] >= ms->mvUserSettings.mvMinimumCI && ms->mvpStateVars->mvpHasNeighNonInd[indexNbrOther] == 0 && ms->mvpStateVars->mvpHasNeighNonInd[indexDragged] == 0)) {
                            if (fabs(ms->MisorientationBetweenCells(indexNbrToGrow, indexDragged, true, false, false, &CSLRelationship)) < MisTol && fabs(ms->MisorientationBetweenCells(indexNbrOther, indexDragged, true, false, false, &CSLRelationship)) >= MisTol) {
                                EnergyPerVolNext *= (1. - ms->mvUserSettings.mvRexGGParameterC);
                            }
                        }
                        NextStateEnergyPerUnitVolume += EnergyPerVolNext;
                    }
                }

                DG_ForReorientation = CurrentStateEnergyPerUnitVolume - NextStateEnergyPerUnitVolume;
                
                if (DG_ForReorientation <= 0.) continue;
                
                TotalForceX *= DG_ForReorientation;
                TotalForceY *= DG_ForReorientation;
                TotalForceZ *= DG_ForReorientation;
                TotalForceForReOrientation = sqrt(pow(TotalForceX, 2.) + pow(TotalForceY, 2.) + pow(TotalForceZ, 2.));

                MobilityOfBoundaries = ms->GetMobilityForThisMisorientation(ms->mvpStateVars->GetLatticeIdOfCell(index), mis[nbr][nbr]);

                fDot = TotalForceForReOrientation * MobilityOfBoundaries;
                fDot *= ms->BetaFactorForInterfaceMigrationRates;
                if (ms->mvUserSettings.mvIncludeCSL19FastGrowth && ms->mvpStateVars->mvpCSLRelationshipNow[index] == 0)
                    fDot *= 0.1;

                if (fDot <= 0.) continue;
                
                if (fDot > 0. && ms->mvpStateVars->mvpConsumptionRate[index] <= 0.)
                    args->localMovingNew++;

                if (fDot <= ms->mvpStateVars->mvpConsumptionRate[index])
                    continue;
                
                // 寫入速率結果 (Thread safe because index is unique)
                ms->mvpStateVars->mvpConsumptionRate[index] = fDot;
                ms->mvpStateVars->mvpOneOfTheNeighboursGrowingInto[index] = indexNbrToGrow;
            }
            
            if (ms->mvpStateVars->mvpConsumptionRate[index] < 0.0) ms->mvpStateVars->mvpConsumptionRate[index] = 0.;
            
            // 更新局部最大值
            if (ms->mvpStateVars->mvpConsumptionRate[index] > args->localMaxRate)
                args->localMaxRate = ms->mvpStateVars->mvpConsumptionRate[index];
        } // End for batch loop
    } // End while loop (Dynamic Scheduling)
...
{
    // 1. 準備資料: Set -> Vector
    std::vector<int> interfaceIndicesVec(interfaceIndices.begin(), interfaceIndices.end());
    
    // 2. 決定執行緒數量
    int numThreads = 4; // 預設值
    const char* env_p = std::getenv("OMP_NUM_THREADS"); // 借用 OMP 的環境變數
    if (env_p) {
        numThreads = std::stoi(env_p);
    }
    if (numThreads < 1) numThreads = 1;

    LOG_F(INFO, "Starting Pthreads execution with %d threads...", numThreads);

    // 3. 初始化 Pthreads 結構
    pthread_t threads[numThreads];
    ThreadArgs threadData[numThreads];
    std::atomic<int> globalIndex(0); // 原子計數器，用於動態排程

    // 4. 建立並啟動執行緒
    for(int i = 0; i < numThreads; i++) {
        threadData[i].instance = this;
        threadData[i].latticeId = latticeId;
        threadData[i].indices = &interfaceIndicesVec;
        threadData[i].globalIndex = &globalIndex;
        
        int rc = pthread_create(&threads[i], nullptr, Microstructure::ReorientationWorker, (void*)&threadData[i]);
        if (rc) {
            LOG_F(ERROR, "Error: unable to create thread, %d", rc);
            exit(-1);
        }
    }

    // 5. 等待執行緒結束並彙整結果 (Reduction)
    float finalMaxRate = 0.0;
    int totalICells = 0;
    int totalMovingPre = 0;
    int totalMovingNew = 0;

    for(int i = 0; i < numThreads; i++) {
        pthread_join(threads[i], nullptr);
        
        // Reduction: Max
        if (threadData[i].localMaxRate > finalMaxRate) {
            finalMaxRate = threadData[i].localMaxRate;
        }
        
        // Reduction: Sum
        totalICells += threadData[i].localICells;
        totalMovingPre += threadData[i].localMovingPre;
        totalMovingNew += threadData[i].localMovingNew;
    }

    LOG_IF_F(INFO, totalICells > 0, "number of iCells %d for lattice id %d for max mob in [meter4 / J s] %g sigma Max %g and kinetic factor beta %g fDotMax %g A/V %g cells moving pre %d and now also %d",
             totalICells,
             latticeId,
...
double meanXSn = 0.;
 ...
 meanXSn /= mvpStateVars->mvN;
   ...
   int NReoriented = TransformCellsFromSolidification();
    ...
    {
        mvImageCount++;
        LOG_F(INFO, "Exporting Output Set: %d", mvImageCount);
