...
extern "C" void LaunchCudaReorientation(
    ...
    const int* h_oriId,
    const float* h_eulerAngles, // Flattened Euler Angles
    int numOris,                // 總取向數
    const float* h_ci,
    const int* h_rx,
...
float Microstructure::CalcReorientationRatesForFullFieldRexOrGG(int latticeId)
{
    ...
    if (numInterfaceCells == 0) return 0.0f;

    LOG_F(INFO, "Starting CUDA execution for %d interface cells...", numInterfaceCells);

    // 2. [CUDA] 準備 Euler Angles 陣列 (Flatten)
    // 假設 mvOrientations 有一個方法可以取得所有角度，或者是 public 的
    // 如果它是 private 的，您可能需要去 microstructure.h 把 mvOrientations 改成 public，
    // 或者去 orientation.h 把 mvOrientation 改成 public。
    // 這裡假設我們可以直接存取 mvOrientations.mvOrientation (std::vector<Eigen::Vector3f>)
    
    // 為了避免修改太多 header，我們先用一個簡單的迴圈把數據拷貝出來
    // 注意：這需要 mvOrientations 提供存取介面。如果沒有，請在 orientation.h 加一個 getter。
    // 假設 orientation.h 有 `const std::vector<Eigen::Vector3f>& GetOrientations() const;`
    // 或者我們直接用 `GetEulerAngles(id)` 慢慢抓
    
    int numOris = mvpStateVars->mvN; // 假設取向ID與Cell數相同，或根據實際取向庫大小
    // 修正：實際上取向庫大小可能比 mvN 小或大，取決於是否共用取向
    // 這裡我們先假設取向ID最大值不會超過 mvN 太大，建立一個足夠大的陣列
    // 更安全的做法是找出最大的 OriId
    int maxOriId = 0;
    for(int i=0; i<mvpStateVars->mvN; ++i) {
        if(mvpStateVars->mvpOriId[i] > maxOriId) maxOriId = mvpStateVars->mvpOriId[i];
    }
    numOris = maxOriId + 1;

    std::vector<float> flatEulerAngles(numOris * 3);
    for (int i = 0; i < numOris; ++i) {
        Eigen::Vector3f ea = mvOrientations.GetEulerAngles(i);
        flatEulerAngles[i * 3 + 0] = ea[0];
        flatEulerAngles[i * 3 + 1] = ea[1];
        flatEulerAngles[i * 3 + 2] = ea[2];
    }

    // 3. [CUDA] 呼叫 GPU Kernel
    LaunchCudaReorientation(
        numInterfaceCells,
        interfaceIndicesVec,
        mvpStateVars->mvN,
        mvpStateVars->mvpLatticeId,
        mvpStateVars->mvpOriId.data(),
        flatEulerAngles.data(),
        numOris,
        mvpStateVars->mvpCI,
        mvpStateVars->mvpRX,
        // 輸出直接寫回 mvpConsumptionRate
        mvpStateVars->mvpConsumptionRate, 
        mvpStateVars->mvpOneOfTheNeighboursGrowingInto,
        // 參數包
        latticeId,
        mvpStateVars->mvNx, mvpStateVars->mvNy, mvpStateVars->mvNz, mvpStateVars->mvDx,
        mvUserSettings.mvMinimumCI, 
        mvUserSettings.mvLowerMisorientationCutOff, 
        mvUserSettings.mvLowerMisorientationForLAGB, 
        mvUserSettings.mvHAGB,
        mvUserSettings.mvGrainBoundaryEnergy, 
        GetMobilityForThisMisorientation(latticeId, mvUserSettings.mvHAGB), // 傳入最大 Mobility
        BetaFactorForInterfaceMigrationRates
    );

    // 4. [CPU] Post-processing (Reduction for Max Rate)
    // 因為 CUDA 那邊還沒做 Reduction，我們在 CPU 這邊掃描一次結果來找最大值
    float fDotMaxICellsGroup = 0.0f;
    int cellsMovingNew = 0;
    int cellsMovingAlready = 0; // 暫時無法從 GPU 拿回這個統計，設為 0 或略過

    for (int index : interfaceIndicesVec) {
        float rate = mvpStateVars->mvpConsumptionRate[index];
        if (rate > fDotMaxICellsGroup) {
            fDotMaxICellsGroup = rate;
        }
        if (rate > 0.0f) cellsMovingNew++;
    }

    LOG_F(INFO, "CUDA Result: MaxRate %g, Cells Moving %d", fDotMaxICellsGroup, cellsMovingNew);
